...
const CentrosFP = () => {
    ...
    const centrosFP = centrosData.centros;

    const centrosPorMunicipio = centrosData.centros.reduce((acc, centro) => {
        const municipio = centro.municipio;
        if (!acc[municipio]) {
            acc[municipio] = [];
        }
        acc[municipio].push(centro);
        return acc;
    }, {});
    ...
    return (
        <>
            ...
            <div className="bg-gray-100 min-h-screen w-full overflow-x-hidden">
                ...
                {Object.entries(centrosPorMunicipio).map(([municipio, centros]) => (
                    <div key={municipio} className="centros-content">
                        <h1>{municipio}</h1>
                        <div className="centros">
                            {centros.map((centro) => (
                                <div key={centro.codigo} className="centro-card">
                                    <div className="centro-img">
                                        <img src={centro.imagen} alt="No hay imágen"/>
                                    </div>
